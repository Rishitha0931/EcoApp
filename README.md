#  EcoApp

> **Making Environmental Learning Fun and Impactful Through Technology**

EcoApp is a **Flutter + Firebase** powered gamified environmental learning platform that transforms sustainability education into an engaging digital experience through eco-challenges, quizzes, rewards, and role-based learning.

Built as a **Hackathon Project**, EcoApp encourages students to develop eco-friendly habits while enabling teachers and administrators to monitor participation and community impact through a centralized platform.

---

#  About The Project

Traditional environmental education often focuses on theory, leaving students with limited opportunities to practice sustainable habits in their daily lives.

EcoApp bridges this gap by providing a digital platform where students actively participate in environmental activities, complete quizzes, earn rewards, and compete through gamified challenges. Teachers create educational content and verify activities, while administrators oversee the entire platform.

By combining **education**, **technology**, and **gamification**, EcoApp makes environmental learning practical, engaging, and impactful.

---

#  Problem Statement

Many students learn about environmental sustainability only through textbooks, resulting in low engagement and minimal real-world participation.

There is a need for an interactive platform that motivates students to actively practice environmentally responsible behaviors while making learning enjoyable and measurable.

EcoApp addresses this challenge through gamification, role-based learning, and real-world eco activities.

---

#  Why EcoApp?

EcoApp encourages students to become active participants in protecting the environment instead of passive learners.

The platform helps:

-  Promote environmental awareness
-  Encourage sustainable habits
-  Increase engagement through gamification
-  Support schools in environmental education
-  Build collaborative learning communities
-  Track student progress and achievements

---

#  Key Features

##  Student Module

- Secure Registration & Login
- Explore Eco Lessons
- Participate in Eco Challenges
- Complete Quizzes
- Upload Environmental Activities
- Track Progress
- Earn Reward Points
- Unlock Achievement Badges
- Maintain Daily Streaks
- Join Team Challenges
- View Leaderboards
- Share Achievements

---

##  Teacher Module

- Teacher Authentication
- Create Learning Content
- Publish Lessons
- Create & Manage Quizzes
- Review Student Activities
- Verify Challenge Submissions
- Monitor Student Progress
- Encourage Classroom Participation

---

##  Admin Module

- Manage Users
- Monitor Platform Activities
- Verify Environmental Campaigns
- Track Student Participation
- Manage Platform Data
- Analyze Community Engagement

---

#  Gamification Features

EcoApp keeps students motivated through interactive learning experiences.

-  Reward Points
-  Achievement Badges
-  Daily Streaks
-  Progress Tracking
-  Leaderboards
-  Team Challenges
-  Milestone Rewards

Gamification encourages consistent participation while making sustainability education enjoyable.

---

#  Eco Challenges

Students participate in practical environmental activities including:

-  Tree Plantation
-  Recycling Challenges
-  Community Clean-Up Drives
-  Energy Saving Activities
-  Water Conservation
-  Green Lifestyle Missions

Every completed activity contributes toward both personal growth and community impact.

---

#  Application Workflow

##  Student Flow

Register/Login → Explore Lessons → Complete Quizzes → Participate in Eco Challenges → Upload Activity → Teacher Verification → Earn Rewards → Climb Leaderboards

---

##  Teacher Flow

Login → Create Lessons → Publish Quizzes → Review Student Activities → Verify Submissions → Monitor Student Progress

---

##  Admin Flow

Manage Users → Monitor Activities → Verify Platform Data → Track Community Impact

---

#  Technology Stack

## Frontend

- Flutter
- Dart
- Material Design

## Backend

- Firebase Authentication
- Cloud Firestore
- Firebase Hosting

## Development Tools

- VS Code
- Android Studio
- Git
- GitHub
- Firebase Console

---

# 🏗 Development Methodology

The project was developed following a structured software development approach.

1. Requirement Analysis
2. UI/UX Design
3. Flutter Development
4. Firebase Integration
5. Role-Based Authentication
6. Testing & Debugging
7. Deployment
8. Continuous Improvements

---

# 📸 Application Screenshots

Explore some of the key screens of **EcoApp**, showcasing its intuitive user interface, role-based functionality, gamified learning experience, and environmental engagement features.

---
# 📸 Application Screenshots

EcoApp offers a clean, intuitive, and engaging user experience designed for students, teachers, and administrators. Below are some of the key screens showcasing the application's features and workflow.

---

## 🌿 Authentication

<p align="center">
  <img src="https://github.com/user-attachments/assets/aaebdd33-b5c5-4e37-a452-b45cb2b60b52" width="220" alt="Splash Screen"/>
  <img src="https://github.com/user-attachments/assets/5c7d8e3e-7ac0-46e0-8997-c08825ac30e5" width="220" alt="Login Screen"/>
</p>

<p align="center">
<b>Splash Screen</b> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<b>Login Screen</b>
</p>

---

## 🏠 Dashboard & Learning

<p align="center">
  <img src="https://github.com/user-attachments/assets/61886382-5991-479f-9326-cb17a78407b3" width="220" alt="Home Screen"/>
  <img src=<img width="540" height="980" alt="lessons_screen" src="https://github.com/user-attachments/assets/2d47d3ab-b8f6-4365-b365-ada93b34b2f3" />

</p>

<p align="center">
<b>Home Dashboard</b> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<b>Interactive Lessons</b>
</p>

---

## 🎯 Daily Eco Activities

<p align="center">
  <img src="https://github.com/user-attachments/assets/5a7027fe-c3b8-44e5-9fbb-7e9b46063dd5" width="220" alt="Today's Tasks"/>
  <img src="https://github.com/user-attachments/assets/c425530b-f2cd-4b6e-8ac2-ac558ca272f0" width="220" alt="Sorting Game"/>
</p>

<p align="center">
<b>Today's Tasks</b> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<b>Waste Sorting Game</b>
</p>

---

## 🤝 Community & Profile

<p align="center">
  <img src="https://github.com/user-attachments/assets/78bd54d9-4aee-49be-b9be-f7234f66f64f" width="220" alt="Community Feed"/>
  <img src="https://github.com/user-attachments/assets/807c7024-9ffc-486c-a88f-bbb26aba9e9c" width="220" alt="Profile"/>
</p>

<p align="center">
<b>Community Feed</b> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<b>Profile Dashboard</b>
</p>

---

## 📅 Eco Events

<p align="center">
  <img src="https://github.com/user-attachments/assets/a267d3cb-fa1e-421d-8150-f3560677289d" width="220" alt="Upcoming Events"/>
  <img src="https://github.com/user-attachments/assets/13a16827-f832-46ee-8c30-2276623ebd21" width="220" alt="Events Calendar"/>
</p>

<p align="center">
<b>Upcoming Events</b> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<b>Events Calendar</b>
</p>

---

### ✨ User Experience Highlights

- 🌱 Clean and nature-inspired interface
- 🎮 Gamified learning with eco challenges and rewards
- 📚 Interactive lessons and quizzes
- 👩‍🏫 Dedicated dashboards for Students, Teachers, and Admins
- 🏆 Progress tracking, achievements, and leaderboards
- 🌍 Community engagement through environmental activities
#  Impact

EcoApp creates positive educational and environmental impact by:

-  Encouraging sustainable habits
-  Promoting environmental responsibility
-  Increasing student engagement
-  Building teamwork and collaboration
-  Supporting digital education
-  Improving participation through rewards
-  Creating community-wide environmental awareness

---

#  Feasibility

## Technical Feasibility

- Cross-platform Flutter application
- Firebase cloud backend
- Responsive user interface
- Offline-first support with synchronization

### Operational Feasibility

- Easy to use
- Suitable for schools and educational institutions
- Role-based access control
- Scalable architecture

---

#  Challenges

During development, several challenges were identified:

- Fake activity submissions
- Student participation gaps
- Internet connectivity limitations
- Data privacy concerns

## Solutions

- Teacher Verification
- Gamification
- Offline Synchronization
- Firebase Authentication
- Secure Cloud Firestore Database

---

#  Future Enhancements

-  AI-powered Eco Recommendations
-  Carbon Footprint Tracker
-  GPS-Based Eco Missions
-  Advanced Analytics Dashboard
-  Push Notifications
-  Multi-language Support
-  School vs School Competitions
-  Digital Achievement Certificates
-  Mobile Application Release

---

#  What I Learned

Developing EcoApp provided valuable hands-on experience in:

- Flutter Development
- Dart Programming
- Firebase Authentication
- Cloud Firestore
- Role-Based Architecture
- UI/UX Design
- Gamification Principles
- Git & GitHub
- Software Testing
- Problem Solving
- Team Collaboration
- Hackathon Development

---

#  Developer

**Rishitha Nallamadhi**

Computer Science Engineering (AI & ML)

### Skills

- Flutter
- Dart
- Firebase
- Git
- GitHub
- UI/UX Design
- Problem Solving

---

#  Hackathon Journey

EcoApp was developed as a hackathon project with the vision of making environmental education more engaging through technology.

Although our team couldn't proceed to the final round due to scheduling constraints, the experience significantly strengthened my skills in Flutter development, Firebase integration, UI/UX design, role-based architecture, and collaborative software development.

The project inspired me to continue improving EcoApp with new features, better analytics, and enhanced learning experiences.

---

#  Live Demo

**Web Application**

https://eco-app-82f71.web.app

---

#  Project Status

 **Actively maintained and continuously improved.**

Upcoming improvements include AI-powered recommendations, enhanced analytics, advanced gamification, and new eco-challenges.

---

⭐ **If you found this project interesting or helpful, consider giving it a Star on GitHub!**
